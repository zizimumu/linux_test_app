...
{
	struct pollfd	fd[1];
	unsigned int count = 0, received = 0,i;
	int fd_log;

		
	for(i=0;i<sizeof(comp_buf);i++){
		comp_buf[i] = i;
	}
	
	fd_log = open("/var/printer_log.bin",O_RDWR|O_CREAT|O_TRUNC);
	if (fd_log < 0) {
	...
	while (1) {
		...
		if (retval && (fd[0].revents & POLLRDNORM)) {
			...
			if (bytes_read < 0) {
				...
				if(bytes_read == 512 || bytes_read == 302){
					if(compare_buf(buf,comp_buf,bytes_read)){
						printf("check error \n");
			...
			count++;
			if((count % 500) == 0)
				printf("receive %d bytes, total %d\n",bytes_read,received);
